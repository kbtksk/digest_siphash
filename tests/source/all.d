    ...
    static foreach (n, T; AliasSeq!(ulong, uint)) {
        static foreach (ver; AliasSeq!(8*T.sizeof, 2*8*T.sizeof)) {
            for (int i = 0; i < 64; i++) {
                ...
                auto r = siphash!(T, ver)(key[0 .. 2*T.sizeof], input[0 .. i]);
                assert(check!(ver)(r, mixin(databases[n] ~ ver.to!string)[i]));
            }
        }
    }
}
...
/// ditto
const ubyte[16][64] vectors_sip128 =
[
...
/// ditto
const ubyte[4][64] vectors_hsip32 =
[
    [ 0xa9, 0x35, 0x9f, 0x5b, ],
    [ 0x27, 0x47, 0x5a, 0xb8, ],
    [ 0xfa, 0x62, 0xa6, 0x03, ],
...
/// ditto
const ubyte[8][64] vectors_hsip64 =
[
